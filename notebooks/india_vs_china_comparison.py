...
india_data = df_long[df_long['Country'].str.contains('India', case=False, na=False)]
china_data = df_long[df_long['Country'].str.contains('China', case=False, na=False)]

# Plot India
if not india_data.empty:
    sns.lineplot(x='Year', y='Expenditure', data=india_data, marker='o')
    plt.title('India Military Expenditure Over Time')
    plt.xlabel('Year')
...
plt.title('Military Expenditure Comparison: India vs China')
plt.xlabel('Year')
